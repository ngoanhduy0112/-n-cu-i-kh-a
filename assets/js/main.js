...
let getDataCategry = (list, cat) => {
  return list.filter((item) => {
    return item.category === cat;
  });
};
const listDataTt = getDataCategry(data, "Thời trang");
const listDataTs = getDataCategry(data, "Trang sức");
const listDataDt = getDataCategry(data, "Điện tử");
...
const ListingProduct = (btn, tab) => {
  btnListing.forEach((button, index) => {
    // console.log(button);
